...
| `--model <model>` | Model for the current session. Provide an alias (e.g. `sonnet`, `opus`) or full model name (e.g. `claude-sonnet-4-6`) |
...
| `--permission-mode <mode>` | Permission mode to use for the session (`acceptEdits`, `bypassPermissions`, `default`, `dontAsk`, `plan`) |
| `--plugin-dir <paths...>` | Load plugins from directories for this session only (repeatable) |
| `-p, --print` | Print response and exit (useful for pipes). The workspace trust dialog is skipped in `-p` mode — only use in directories you trust |
...
| `--tools <tools...>` | Specify the list of available tools. Use `""` to disable all, `"default"` for all tools, or specify names (e.g. `"Bash,Edit,Read"`) |
...
| `install [options] [target]` | Install Claude Code native build. Use `[target]` to specify version (`stable`, `latest`, or a specific version) |
